...
class Team:
    def __init__(self, tid, defcon_id, ai, team_name):
        self.tid = tid
...
class State:
    def __init__(self, teams_with_ais, boardradius, boardradius_inner, teamradius, defcon_round, seed, buildteams, gamentick, defcon_ids_list=None):
        self.tick = 0
        self.boardradius = boardradius
        self.boardradius_inner = boardradius_inner
        self.events = []
        self.teams = {}
        self.teams_by_defcon_id = {}
        self.defcon_round = defcon_round
        self.seed = seed
        self.moves = None
        self.gamentick = gamentick
        if buildteams:
            if defcon_ids_list is None:
                defcon_ids = teams_with_ais.keys()
                defcon_ids_list = list(defcon_ids)
                random.shuffle(defcon_ids_list)
            for tid, defcon_id in enumerate(defcon_ids_list):
                team_name, team_ai = teams_with_ais[defcon_id]
                t = Team(tid, defcon_id, team_ai, team_name)
                t.ship = Ship()
                t.ship.location.move(((math.pi*2.0)/(len(teams_with_ais)))*tid, teamradius)
                t.ship.location.move(math.pi)
                self.teams_by_defcon_id[defcon_id] = t
                self.teams[tid] = t
